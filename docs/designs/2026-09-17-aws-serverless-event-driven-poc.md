...
| D9 | **Amplify is created in the console from GitHub, and is NOT in Terraform.** **Superseded the original plan on 2026-09-24.** | The original plan was "Terraform declares the app, connect the repository once by hand". **That step does not exist.** The console offers repository connection only when an app is CREATED, never for an existing one, so an app Terraform made can never be connected. The provider cannot do it either: `oauth_token` wires the deprecated OAuth path, `access_token` takes only classic `ghp_` tokens, and CodeConnections support is still an open enhancement request (hashicorp/terraform-provider-aws#32610). So the app is created in the console from GitHub and builds on push. `infra/main/main.tf` carries a comment warning against adding `aws_amplify_app` back: it would create a second, empty app beside the working one. | Terraform + manual connect (**the step does not exist**). A classic `ghp_` PAT in Terraform (long-lived credential, and GitHub is retiring them). S3 + CloudFront (fully Terraform-managed and cheaper, and the right answer beyond a POC - rejected only because the architecture was already proven and this is presentation). |
...
| D18 | **Dark mode follows the operating system. There is no theme toggle.** `@custom-variant dark (@media (prefers-color-scheme: dark))` in `web/src/index.css`. | shadcn/ui ships a class-based dark mode that expects a toggle to add `.dark` to the root. This app has no toggle, so without the redefinition every `dark:` utility would be dead code and the page would stay light on a dark desktop. One line restores the whole set. **Note: `shadcn init` overwrites the colour values in `index.css` and leaves your comments in place**, so they end up describing colours that are no longer there. Re-check the file after any `init`. | A theme toggle (state, storage and a control, for a three-element page). Accepting shadcn's class-based default (every `dark:` utility silently dead). |
| D17 | **HTTP API CORS stays `allow_origins = ["*"]`. It is NOT narrowed to the Amplify domain.** | Two reasons. Narrowing it creates a Terraform dependency cycle: the ingest API would depend on the Amplify app, which depends on the ingest stage URL for its `VITE_API_URL`. More importantly, the endpoint has **no authentication** (D12), and CORS is enforced by the browser rather than the server — anyone can call the API with `curl` whatever `allow_origins` says. A narrow value would look like a control while protecting nothing. Narrow it at the same time auth is added, never before. | Narrowing to the Amplify domain (cycle, plus false assurance). Breaking the cycle with a hand-set variable (a manual step for no security gain). |
